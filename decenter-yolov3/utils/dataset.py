...
from utils.utils import read_class_names, image_preprocess, bbox_iou, YOLO_ANCHORS, YOLO_STRIDES


class Dataset(object):
    # Dataset preprocess implementation
    def __init__(self, dataset_type, annoth_path):
        self.annot_path = annoth_path
        self.input_sizes = 416
        self.batch_size = 4
        self.data_aug = dataset_type == 'train'

        self.train_input_sizes = 416
        self.strides = np.array(YOLO_STRIDES)
        self.classes = read_class_names("model_data/names.txt")
    ...
    def load_annotations(self, dataset_type):
        ...
        with open(self.annot_path, 'r') as f:
            txt = f.readlines()
            annotations = ["train_data/" + line.strip() for line in txt if len(line.strip().split()[1:]) != 0]
